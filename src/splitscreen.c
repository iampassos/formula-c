...
void *loadSplitscreen(void *arg) {
    ...
    strcpy(load_msg, "Carregando som...");
    semaphoreSound = LoadSound(SMAPHORE_SOUND_PATH);

    strcpy(load_msg, "Carregando carros...");
    Car *p1 = Car_create(map->startCarPos[0], map->startAngle, DEFAULT_CAR_CONFIG, &carsImage[1],
                         BLUE, false, 1, "Player 1");
    LinkedList_addCar(cars, p1);

    Car *p2 = Car_create(map->startCarPos[1], map->startAngle, DEFAULT_CAR_CONFIG, &carsImage[2],
                         ORANGE, false, 2, "Player 2");
    LinkedList_addCar(cars, p2);

    strcpy(load_msg, "Carregando cameras...");
