...
void Client_Draw() {
    ...
    Car_showInfo(player, player->pos.x - SCREEN_WIDTH / 2, player->pos.y - SCREEN_HEIGHT / 2, 50,
                 BLACK);

    Car_list_transfer nearCars = Server_GetNearCars(playerId);
    for (int i = 0; i < nearCars.length; i++) {
        Car_draw(&nearCars.data[i]);
    }
