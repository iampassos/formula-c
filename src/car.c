...
void Track_setDrag(float track_drag, float light_escape_area_drag,
                   float hard_escape_area_drag, float ouside_track_drag) {
...
static bool isOnTrack(Color color) {
    return color.r == TRACK_COLOR.r && color.g == TRACK_COLOR.g &&
           color.b == TRACK_COLOR.b;
}

static bool isOnLightEscapeArea(Color color) {
    return color.r == LIGHT_ESCAPE_AREA_COLOR.r &&
           color.g == LIGHT_ESCAPE_AREA_COLOR.g &&
           color.b == LIGHT_ESCAPE_AREA_COLOR.b;
}

static bool isOnHardEscapeArea(Color color) {
    return color.r == HARD_ESCAPE_AREA_COLOR.r &&
           color.g == HARD_ESCAPE_AREA_COLOR.g &&
           color.b == HARD_ESCAPE_AREA_COLOR.b;
}

static bool isOutSideTrack(Color color) {
    return color.r == OUTSIDE_TRACK_COLOR.r &&
           color.g == OUTSIDE_TRACK_COLOR.g && color.b == OUTSIDE_TRACK_COLOR.b;
}
...
void Car_accelerate(Car *car) { car->vel += car->acc; }
...
void Car_turnLeft(Car *car) { Car_turn(car, -car->angularAcc); }

void Car_turnRight(Car *car) { Car_turn(car, car->angularAcc); }

void Car_break(Car *car) { car->vel *= car->breakForce; }

void Car_reverse(Car *car) { car->vel -= car->acc * 0.2; }
...
void Car_setPos(Car *car, Vector2 newPos) { car->pos = newPos; }

void Car_setAngle(Car *car, float angle) { car->angle = angle; }
...
void Car_info(Car *car) {
    printf("Carro id: %d\n"
           ...
           "breakForce: %.2f\n"
           "dragForce: %.2f\n",
           car->id, car->lapTime, car->pos.x, car->pos.y, car->vel, car->acc,
           car->width, car->height, car->color.r, car->color.g, car->color.b,
           car->color.a, car->angle, car->angularAcc, car->minTurnSpeed,
           car->breakForce, car->dragForce);
}
