...
void Track_setDrag(float track_drag, float light_escape_area_drag, float hard_escape_area_drag, float ouside_track_drag); // Função que define as propriedades da pista
void Track_setMask(Image mask, Color *maskPixel); // Definindo a mascara de pixel para carros lerem
void Track_setColor(Color track, Color light_escape, Color hard_escape, Color outside); // Definindo (rgb) para cada parte da pista

void Track_Unload();

Car* Car_create(Vector2 pos, float acc, int width, int height, Color color, float angle, float angularAcc, float minTurnSpeed, float breakForce,int id);
...
void Car_setAngle(Car* car, float angle); // Definindo um novo ângulo para o carro
